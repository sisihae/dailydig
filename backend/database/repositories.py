...
from backend.models import DigQueue, Feedback, RecommendationHistory, TasteProfile, Track
...
async def upsert_taste_profile(
    session: AsyncSession, user_id: int = 1, **kwargs
) -> TasteProfile:
...
async def create_feedback(
    session: AsyncSession, track_id: int, feedback_type: str
) -> Feedback:
...
async def get_feedback_for_period(
    session: AsyncSession, days: int = 30
) -> list[Feedback]:
    cutoff = datetime.now(timezone.utc) - timedelta(days=days)
    result = await session.execute(select(Feedback).where(Feedback.timestamp >= cutoff))
    return list(result.scalars().all())
