# Plan 10 — Queue Service

**Phase**: 2 – Spotify Integration + Playlist Import  
**Creates**: `backend/services/queue_service.py`  
**Depends on**: 06 (repositories), 09 (spotify service)

---

## Goal

High-level queue management: import playlists (with dedup), pick tracks, check queue state.
...
- Dedup checks both `dig_queue` (any status) and `recommendation_history`.
- Genre fetched from first artist (best-effort).
- Entire playlist import is atomic — committed at the end.

## Verification

After importing a playlist, `get_queue_status()` should show all imported tracks.

## Output

- `backend/services/queue_service.py` — playlist import, random pick, status
