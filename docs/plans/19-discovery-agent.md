# Plan 19 — Discovery Agent

**Phase**: 5 – Auto-Discovery Pipeline  
**Creates**: `backend/agents/discovery_agent.py`  
**Depends on**: 18 (planner strategy), 09 (spotify service), 06 (repositories)

---

## Goal

Fetch 30–50 candidate tracks from Spotify based on the planner's strategy, with dedup filtering.
...
- Passes `seed_artists` and `seed_genres` directly from strategy to Spotify API.
- Dedup checks both `recommendation_history` and `dig_queue`.
- Audio features enriched from Redis-cached Spotify responses.
- Genre fetched per artist (best effort; Spotify doesn't return genre per track).

## Verification

```python
discovery = DiscoveryAgent(SpotifyService())
...
- `backend/agents/discovery_agent.py` — Spotify recommendation fetcher with dedup
