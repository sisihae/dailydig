...
class FeedbackAgent:
    ...
    def parse_callback_data(callback_data: str) -> tuple[str, int]:
        ...
        feedback_type = parts[0]  # "like", "dislike", "skip"
        track_id = int(parts[1])
        return feedback_type, track_id

    async def process_feedback(self, feedback_type: str, track_id: int) -> dict:
        """
        Store feedback in DB and trigger taste update.
        ...
        # Taste update is triggered separately (Plan 16)
...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # Store feedback
    result = await feedback_agent.process_feedback(feedback_type, track_id)

    # Update taste profile
    taste_agent = TasteModelAgent()
    await taste_agent.update_from_feedback(feedback_type, track_id)

    # Confirm to user
    emoji_map = {"like": "👍", "dislike": "👎", "skip": "⏭"}
    emoji = emoji_map.get(feedback_type, "✓")
    await query.edit_message_reply_markup(reply_markup=None)  # remove buttons
    await query.message.reply_text(f"{emoji} Feedback recorded! Thanks for helping me learn your taste.")
...
async def submit_feedback(body: FeedbackRequest, session: AsyncSession = Depends(get_session)):
    ...
    agent = FeedbackAgent()
    result = await agent.process_feedback(body.feedback_type, body.track_id)

    taste_agent = TasteModelAgent()
    await taste_agent.update_from_feedback(body.feedback_type, body.track_id)
